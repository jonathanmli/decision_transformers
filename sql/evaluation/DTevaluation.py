from sql.evaluation.RLevaluation import Evaluater
import torch
import numpy as np

class DT_Evaluater(Evaluater):
    def evaluate(self, env, model, dtype=torch.float, itype=torch.int, normalized=False, sequence_length = 20, n_eps = 100, target_return=3600, max_ep_len=1000, scale=1000, device='cpu', state_mean=None, state_std=None, mode='normal'):
...
class BM_Evaluater(Evaluater):
    ...
    '''
    def evaluate(self, env, model, sequence_length = 20, n_eps = 100, target_return=3600, max_ep_len=1000, scale=1000, device='cpu', state_mean=None, state_std=None, mode='normal'):
        state_dim = np.prod(env.observation_space.shape)
        action_dim = env.action_space.shape[0]
